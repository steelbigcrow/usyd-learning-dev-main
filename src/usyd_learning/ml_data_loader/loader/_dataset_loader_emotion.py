...
class DatasetLoader_Emotion(DatasetLoader):
    """
    HF emotion: 字段(text, label)
    ...
    def _create_inner(self, args: DatasetLoaderArgs) -> None:
        ds = load_dataset("dair-ai/emotion", cache_dir=getattr(args, "root", None))
        split = args.split or "train"
        if split not in ds:
            raise ValueError(f"emotion split '{split}' not found. Available: {list(ds.keys())}.")
