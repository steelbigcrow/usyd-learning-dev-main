# usyd_learning/ml_data_loader/loader/_dataset_loader_flowers102.py
...
class _HF_Flowers102_Wrapper(Dataset):
...
class DatasetLoader_Flowers102(DatasetLoader):
    def __init__(self):
        super().__init__()
    ...
    def _create_inner(self, args: DatasetLoaderArgs) -> None:
        ...
        if not _HAS_HF:
            raise ImportError("Flowers102 requires 'datasets' library. Please `pip install datasets`.")

        cache_dir = getattr(args, "root", None)
        # Try loading from tensorflow_datasets (requires tensorflow-datasets package)
        try:
            ds = load_dataset("tensorflow_datasets", "oxford_flowers102", cache_dir=cache_dir)
        except Exception as e:
            raise ImportError(
                f"Failed to load Flowers102 dataset: {e}. "
        ...
        if split not in ds:
            raise ValueError(f"Flowers102 split '{split}' not found. Available: {list(ds.keys())}.")

        # 训练集
        if split == "train":
            train_ds = _HF_Flowers102_Wrapper(ds["train"], transform)
        ...
        # 测试集
        if "test" in ds:
            test_ds = _HF_Flowers102_Wrapper(ds["test"], test_transform)
            self._test_dataset = test_ds
            self._test_data_loader = DataLoader(
                test_ds, batch_size=test_bs, shuffle=False, num_workers=nw
            )
        elif "validation" in ds:
            val_ds = _HF_Flowers102_Wrapper(ds["validation"], test_transform)
