# ml_data_loader/loader/_dataset_loader_sst2.py
...
try:
    ...
    raise ImportError(
        "HuggingFace 'datasets' is required for SST2. Please `pip install datasets`."
    ) from e
...
class DatasetLoader_SST2(DatasetLoader):
    ...
    def _create_inner(self, args: DatasetLoaderArgs) -> None:
        if getattr(args, "text_collate_fn", None) is None:
            raise ValueError("SST2 requires args.text_collate_fn (bind your tokenizer & vocab).")

        ds = load_dataset("glue", "sst2", cache_dir=getattr(args, "root", None))
