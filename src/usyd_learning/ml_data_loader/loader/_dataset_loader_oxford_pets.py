# usyd_learning/ml_data_loader/loader/_dataset_loader_oxford_pets.py
...
class _HF_OxfordPets_Wrapper(Dataset):
...
class DatasetLoader_OxfordPets(DatasetLoader):
    ...
    def _create_inner(self, args: DatasetLoaderArgs) -> None:
        ...
        if not _HAS_HF:
            raise ImportError("OxfordPets requires 'datasets' library. Please `pip install datasets`.")

        cache_dir = getattr(args, "root", None)
        # Try loading from tensorflow_datasets (requires tensorflow-datasets package)
        try:
            ds = load_dataset("tensorflow_datasets", "oxford_iiit_pet", cache_dir=cache_dir)
        except Exception as e:
            raise ImportError(
                f"Failed to load OxfordPets dataset: {e}. "
        ...
        split = getattr(args, "split", "trainval")
        if split not in ds:
            raise ValueError(f"OxfordPets split '{split}' not found. Available: {list(ds.keys())}.")

        # 训练集
        if split == "trainval" or split == "train":
            train_ds = _HF_OxfordPets_Wrapper(ds[split], transform)
        ...
        # 测试集
        if "test" in ds:
            test_ds = _HF_OxfordPets_Wrapper(ds["test"], test_transform)
