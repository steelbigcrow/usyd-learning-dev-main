import os
import unittest
import torch

# Ensure imports work when running under different IDEs
from startup_init import startup_init_path
startup_init_path(os.path.dirname(os.path.abspath(__file__)))


class TestRBLAAggregator(unittest.TestCase):
    def _make_client_sds(self):
        # Client 0: r = 2
        A0 = torch.tensor(
            [
                [1.0, 1.0, 1.0],  # row 0
                [2.0, 2.0, 2.0],  # row 1
            ],
            dtype=torch.float32,
        )  # [2, 3]
        B0 = torch.tensor(
            [
                [10.0, 20.0],
                [10.0, 20.0],
            ],
            dtype=torch.float32,
        )  # [2, 2]
        W0 = torch.ones((2, 3), dtype=torch.float32)  # non-LoRA param
        sd0 = {
            "layer.lora_A": A0.clone(),
            "layer.lora_B": B0.clone(),
            "layer.weight": W0.clone(),
        }

        # Client 1: r = 4
        A1 = torch.tensor(
            [
                [3.0, 3.0, 3.0],
                [4.0, 4.0, 4.0],
                [5.0, 5.0, 5.0],
                [6.0, 6.0, 6.0],
            ],
            dtype=torch.float32,
        )  # [4, 3]
        B1 = torch.tensor(
            [
                [7.0, 8.0, 9.0, 10.0],
                [7.0, 8.0, 9.0, 10.0],
            ],
            dtype=torch.float32,
        )  # [2, 4]
        W1 = 2.0 * torch.ones((2, 3), dtype=torch.float32)
        sd1 = {
            "layer.lora_A": A1.clone(),
            "layer.lora_B": B1.clone(),
            "layer.weight": W1.clone(),
        }
        return sd0, sd1

    def test_rbla_variable_rank_equal_weights(self):
        from usyd_learning.fl_algorithms.aggregation.methods._fed_aggregator_rbla import (
            FedAggregator_RBLA,
        )
        from usyd_learning.fl_algorithms.aggregation.fed_aggregator_args import (
            FedAggregatorArgs,
        )

        sd0, sd1 = self._make_client_sds()

        print("\n=== [RBLA] Pre-Aggregation (Client 0) ===")
        print("A0 shape:", tuple(sd0["layer.lora_A"].shape))
        print(sd0["layer.lora_A"])
        print("B0 shape:", tuple(sd0["layer.lora_B"].shape))
        print(sd0["layer.lora_B"])
        print("W0 shape:", tuple(sd0["layer.weight"].shape))
        print(sd0["layer.weight"])

        print("\n=== [RBLA] Pre-Aggregation (Client 1) ===")
        print("A1 shape:", tuple(sd1["layer.lora_A"].shape))
        print(sd1["layer.lora_A"])
        print("B1 shape:", tuple(sd1["layer.lora_B"].shape))
        print(sd1["layer.lora_B"])
        print("W1 shape:", tuple(sd1["layer.weight"].shape))
        print(sd1["layer.weight"])

        args = FedAggregatorArgs({"aggregation": {"device": "cpu", "method": "rbla"}})
        aggregator = FedAggregator_RBLA(args)

        aggregated = aggregator.aggregate([(sd0, 1.0), (sd1, 1.0)])

        A_agg = aggregated["layer.lora_A"]
        B_agg = aggregated["layer.lora_B"]
        W_agg = aggregated["layer.weight"]

        print("\n=== [RBLA] Post-Aggregation (NaN-Pad) ===")
        print("A_agg shape:", tuple(A_agg.shape))
        print(A_agg)
        print("B_agg shape:", tuple(B_agg.shape))
        print(B_agg)
        print("W_agg shape:", tuple(W_agg.shape))
        print(W_agg)

        # Shapes should be padded to union (r_max = 4)
        self.assertEqual(tuple(A_agg.shape), (4, 3))
        self.assertEqual(tuple(B_agg.shape), (2, 4))
        self.assertEqual(tuple(W_agg.shape), (2, 3))

        # Expected values under RBLA (NaN padding, equal weights -> average where both exist;
        # missing dims reduce the denominator to only valid weights -> value equals the present client's value)
        A_expected = torch.tensor(
            [
                [(1.0 + 3.0) / 2, (1.0 + 3.0) / 2, (1.0 + 3.0) / 2],
                [(2.0 + 4.0) / 2, (2.0 + 4.0) / 2, (2.0 + 4.0) / 2],
                [5.0, 5.0, 5.0],  # only client 1 has row 2
                [6.0, 6.0, 6.0],  # only client 1 has row 3
            ],
            dtype=torch.float32,
        )

        B_expected = torch.tensor(
            [
                [(10.0 + 7.0) / 2, (20.0 + 8.0) / 2, 9.0, 10.0],  # cols 2..3 only from client 1
                [(10.0 + 7.0) / 2, (20.0 + 8.0) / 2, 9.0, 10.0],
            ],
            dtype=torch.float32,
        )

        W_expected = (sd0["layer.weight"] + sd1["layer.weight"]) / 2.0

        self.assertTrue(torch.allclose(A_agg.cpu(), A_expected, atol=1e-6))
        self.assertTrue(torch.allclose(B_agg.cpu(), B_expected, atol=1e-6))
        self.assertTrue(torch.allclose(W_agg.cpu(), W_expected, atol=1e-6))

    def test_rbla_variable_rank_unequal_weights(self):
        from usyd_learning.fl_algorithms.aggregation.methods._fed_aggregator_rbla import (
            FedAggregator_RBLA,
        )
        from usyd_learning.fl_algorithms.aggregation.fed_aggregator_args import (
            FedAggregatorArgs,
        )

        sd0, sd1 = self._make_client_sds()

        # Use unequal volumes -> internal normalization will yield weights [0.25, 0.75]
        vols = (1.0, 3.0)

        args = FedAggregatorArgs({"aggregation": {"device": "cpu", "method": "rbla"}})
        aggregator = FedAggregator_RBLA(args)

        aggregated = aggregator.aggregate([(sd0, vols[0]), (sd1, vols[1])])

        A_agg = aggregated["layer.lora_A"]
        B_agg = aggregated["layer.lora_B"]
        W_agg = aggregated["layer.weight"]

        print("\n=== [RBLA] Post-Aggregation (NaN-Pad, unequal weights 1:3) ===")
        print("A_agg shape:", tuple(A_agg.shape))
        print(A_agg)
        print("B_agg shape:", tuple(B_agg.shape))
        print(B_agg)
        print("W_agg shape:", tuple(W_agg.shape))
        print(W_agg)

        # Shapes should still be padded to union (r_max = 4)
        self.assertEqual(tuple(A_agg.shape), (4, 3))
        self.assertEqual(tuple(B_agg.shape), (2, 4))
        self.assertEqual(tuple(W_agg.shape), (2, 3))

        # With normalized weights w0=0.25, w1=0.75:
        # Overlap rows/cols -> weighted average 0.25*A0 + 0.75*A1
        # Missing dims -> denominator equals the present weight (0.75), numerator equals present*0.75 -> cancels to present value
        A_expected = torch.tensor(
            [
                [0.25 * 1.0 + 0.75 * 3.0] * 3,
                [0.25 * 2.0 + 0.75 * 4.0] * 3,
                [5.0, 5.0, 5.0],
                [6.0, 6.0, 6.0],
            ],
            dtype=torch.float32,
        )

        B_expected = torch.tensor(
            [
                [0.25 * 10.0 + 0.75 * 7.0, 0.25 * 20.0 + 0.75 * 8.0, 9.0, 10.0],
                [0.25 * 10.0 + 0.75 * 7.0, 0.25 * 20.0 + 0.75 * 8.0, 9.0, 10.0],
            ],
            dtype=torch.float32,
        )

        W_expected = 0.25 * sd0["layer.weight"] + 0.75 * sd1["layer.weight"]

        self.assertTrue(torch.allclose(A_agg.cpu(), A_expected, atol=1e-6))
        self.assertTrue(torch.allclose(B_agg.cpu(), B_expected, atol=1e-6))
        self.assertTrue(torch.allclose(W_agg.cpu(), W_expected, atol=1e-6))


if __name__ == "__main__":
    unittest.main()

